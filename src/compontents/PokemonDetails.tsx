...
const PokemonDetails: React.FC<PokemonListProps> = ({ pokemon, loading }) => {
  ...
  useEffect(() => {
    const foundPokemonIndex = pokemon.findIndex(p => p.name === params.pokemonName);
  ...
  const abilites = currentPokemon.abilities.map((ability:any) => (
    ability.ability.name))

  const types = currentPokemon.types.map((type:any) => (
    type.type.name))
  ...
  return (
    ...
    <section className='details'>
      ...
      <Link className="goBackLink" to="/">Go to Pokemon List</Link>
    </div>
  );
};
